...
On the SnapCol grid, the computational mode found in the gravity waves test, described in section~\ref{sec:gw}, is present in this advection test (see figure~\ref{fig:wobblyThetaAdvection:thetaDiff:snapCol}).  This result confirms that it is error in the upwind-biased advection scheme that excites the Lorenz computational mode.

Compared with the gravity wave result in figure~\ref{fig:gw:thetaDiff:snapCol} we find the warm and cold anomalies are more severe in this advection test.  However, it is important to note two differences between the gravity waves test and this advection test.  First, only the advection equation is being solved instead of the fully-compressible Euler equations that were solved in the gravity waves test.  Second, the velocity field that is prescribed does not match exactly the flow in the gravity waves test.
