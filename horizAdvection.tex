...
\begin{figure}
	\captionsetup[subfigure]{position=b}
	\centering
	\includegraphics[height=5in,angle=270]{legends/tracer-contour-mountainPeak_T.eps}
%
	\subcaptionbox{BTF tracer \label{fig:advection:tracer5000:btf}}[0.49\textwidth]{\includegraphics[width=1.8in,angle=270]{openfoam/cases/advection/btf/schaerCos/cubicUpwindCPCFit/5000/tracer-contour-mountainPeak.eps}}
	\hfill
	\subcaptionbox{SLEVE tracer \label{fig:advection:tracer5000:sleve}}[0.49\textwidth]{\includegraphics[width=1.8in,angle=270]{openfoam/cases/advection/sleve/schaerCos/cubicUpwindCPCFit/5000/tracer-contour-mountainPeak.eps}}
%
	\subcaptionbox{BTF divergence \label{fig:advection:div:btf}}[0.49\textwidth]{\includegraphics[width=1.8in,angle=270]{openfoam/cases/advection/btf/schaerCos/cubicUpwindCPCFit/0/divU.eps}}
	\hfill
	\subcaptionbox{SLEVE divergence \label{fig:advection:div:sleve}}[0.49\textwidth]{\includegraphics[width=1.8in,angle=270]{openfoam/cases/advection/sleve/schaerCos/cubicUpwindCPCFit/0/divU.eps}}
%
	\includegraphics[height=5in,angle=270]{legends/divU.eps}
%
	\caption{Tracer magnitude in the centremost \SI{20}{\kilo\meter} and lowest \SI{8}{\kilo\meter} at $t = \SI{5000}{\second}$ on (\subcaptionref{fig:advection:tracer5000:btf}) the BTF grid, and (\subcaptionref{fig:advection:tracer5000:sleve}) the SLEVE grid.  The tracer colour scale, shown above the figures, is narrowed to reveal small magnitude errors.  Divergence (\si{\per\second}) of the discrete velocity field is shown on (\subcaptionref{fig:advection:div:btf}) the BTF grid, and (\subcaptionref{fig:advection:div:sleve}) the SLEVE grid.  Divergence colour scale is shown below the figures.  Results on the SnapCol grid are not shown since tracer errors above the mountain are minimal, and the discrete velocity field is entirely non-divergent.}
	\label{fig:advection:div}
\end{figure}

Divergence was calculated on the BTF, SLEVE and SnapCol grids using linear interpolation.  Divergence is found between about \SI{2}{\kilo\meter} and \SI{6}{\kilo\meter} above the surface, which includes the region of vertical wind shear.  As seen in figure~\ref{fig:advection:div:btf}, divergence is most significant on the BTF grid, with magnitudes reaching $\sim \SI{6e-3}{\per\second}$.  Divergence is also present on the SLEVE grid, but magnitudes do not exceed \SI{8e-4}{\per\second}.
