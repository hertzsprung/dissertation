...
There are some issues with grid construction which mean that this is not strictly a cut cell grid.  First, when \shellcmd{snappyHexMesh} moves points along the surface according to its heuristics, some points are moved horizontally.  Secondly, the utility does not create new points necessary for pentagonal cells, such as those seen in figure~\ref{fig:intro:shaved:piecewise-linear}.  It has not been possible to correct these issues for this project.  Henceforth, this grid is referred to as the `SnapCol' grid.
...
\end{cases}
\end{align}
Equation~\ref{eqn:method:exner-grad} is second-order accurate when the cell face is equidistant from the centres of the two adjoining cells, and first-order accurate otherwise.
