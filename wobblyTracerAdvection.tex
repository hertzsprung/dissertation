...
\TODO{this test tells us if errors are due to mesh non-orthogonality and skewness, or due to alignment of the flow with the mesh}

In the horizontal advection test, results were more accurate when the flow was aligned with grid layers: on the cut cell grid results were accurate, but distortions in the BTF grid led to increased error.  This terrain following advection test is designed to determine the nature of these errors.  They are either caused by skewness or grid non-uniformity (described in section~\ref{sec:skewness}) so that the velocity field is aligned with the layers of the BTF grid.
...
\subsection{Results}
\TODO{be careful talking about errors, because we only know the analytic solution at t=10000s (or do we?)}
...
In contrast, the tracer suffers from significant error on the SnapCol grid as evidenced by the fewer, wider contours in figure~\ref{fig:wobblyTracerAdvection:snapCol}.  The error on the SnapCol grid is larger than that for any other tracer advection.  Tracer amplitude is reduced to \input{openfoam/cases/wobblyTracerAdvection/snapCol/schaerCos/cubicUpwindCPCFit/max.txt} (see figure~\ref{fig:wobblyTracerAdvection:ranges:tf}).  $\ell^2$ error norms are compared with those for horizontal advection in table~\ref{tab:advection:errors}.
...
\TODO{conclude that errors are mainly due to lack of flow alignment with the mesh rather than skewness.  we will need to explain skewness somewhere, too}
